...
Application Cho Viego. It records the planned Streamlit target and its
acceptance boundaries. It does not describe the current interface as
redesigned, implemented, or running in production.
...
## Current frontend versus planned target

| Area | Current frontend | Approved target |
| --- | --- | --- |
...
The current Master Profile, Resume Tailor, and Cover Letter interface is not
Precision Workbench. The target names and modules are planned changes.
