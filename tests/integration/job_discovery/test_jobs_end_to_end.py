...
def _evaluation(
    ...
    *,
    ...
    unresolved = (
        ["Work authorization is not stated."] if eligibility is EligibilityStatus.UNKNOWN else []
    )
...
def test_composed_refresh_api_presentation_and_handoff_are_offline_and_idempotent(tmp_path) -> None:
    ...
    first_run = refresh.refresh("local-user", profile.id, preferences, started_at=NOW)
    ...
    second_run = refresh.refresh("local-user", profile.id, preferences, started_at=NOW)
    ...
    repeated_explore_feed = feed_queries.get("local-user", FeedKind.EXPLORE, profile_id=profile.id)
