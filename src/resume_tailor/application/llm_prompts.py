...
def task_prompt(operation: LlmOperation, request: PromptRequest) -> str:
    task = {
        ...
        LlmOperation.APPLICATION_STRATEGY: (
            ...
            "and a small ranked expansion_reserve of distinct reviewed evidence worth adding only when "
            "the rendered core portfolio is materially underfilled. A reserve action may deepen a core "
            "entry or open one additional relevant entry, but a new professional experience must supply "
            "at least two coherent evidence IDs. Do not repeat core evidence in the reserve. Also provide "
