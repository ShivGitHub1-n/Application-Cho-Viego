...
class DeterministicResumeOptimizer:
    ...
    def create_plan(
        ...
        selected_records = [
            record for package in selected_packages for record in package.candidates
        ]
        selected_entity_ids = [package.entity_id for package in selected_packages]
        covered_signal_ids = (
            set().union(*(package.signal_ids for package in selected_packages))
        ...
        return TailoringPlan(
            ...
            report=DecisionReport(
                ...
                decisions=decisions,
                warnings=[
                    "Only confirmed, evidence-linked claims are eligible for "
    ...
    def _unsupported_plan(
        ...
        return TailoringPlan(
            ...
            constraints=constraints,
            report=DecisionReport(
                role=role, warnings=[role.reason or "The posting could not be classified."]
            ),
        )
    ...
    def _candidate_records(
        ...
        for evidence in profile.evidence:
            ...
            records.append(
                CandidateRecord(candidate, frozenset(signal_ids), self._impact(evidence))
            )
    ...
    def _assess_profile_fit(
        ...
        direct_signal_ids = (
            set().union(*(record.signal_ids for record in records)) if records else set()
        )
        declared_skill_signal_ids = self._matching_declared_skill_signal_ids(
            profile.declared_skills, role
        )
        material_gaps = [
            signal.label
        ...
        primary_direct_evidence = any(
            signal.family.value == role.role_family and signal.id in direct_signal_ids
            for signal in role.signals
        )
        if not records:
            return ProfileFitAssessment(
                ...
                reason=(
                    "The reviewed profile has no confirmed evidence that directly "
        ...
        if material_gaps or not primary_direct_evidence:
            return ProfileFitAssessment(
                ...
                reason=(
                    "The profile has relevant direct evidence, but important posting "
        ...
        return ProfileFitAssessment(
            ...
            declared_skill_signal_ids=sorted(declared_skill_signal_ids),
            reason=(
                "The profile contains direct evidence for the opportunity's primary role signals."
            ),
        )
    ...
    def _entry_packages(
        ...
        for entity_id, entity_records in by_entity.items():
            candidates = sorted(
                entity_records, key=lambda record: (-record.impact, record.candidate.id)
            )[:8]
            overhead = (
            ...
            for count in range(1, min(constraints.max_bullets_per_entry, len(candidates)) + 1):
                for option in combinations(candidates, count):
                    evidence_ids = [
                        ...
                        for evidence_id in record.candidate.evidence_ids
                    ]
                    if len(evidence_ids) != len(set(evidence_ids)):
                        continue
                    signal_ids = frozenset().union(*(record.signal_ids for record in option))
                    line_cost = overhead + sum(
                        record.candidate.estimated_lines for record in option
                    )
                    packages.append(
                        EntryPackage(
                            entity_id=entity_id,
    ...
    def _select_packages(
        ...
        while remaining:
            eligible = [
                package for package in remaining if self._fits(package, selected, constraints)
            ]
            if not eligible:
                break
            best = max(
    ...
    def _fits(
        self, package: EntryPackage, selected: list[EntryPackage], constraints: TemplateConstraints
    ) -> bool:
    ...
    def _all_fit(packages: list[EntryPackage], constraints: TemplateConstraints) -> bool:
        ...
        experience = sum(
            package.line_cost for package in packages if package.kind == EntityKind.EXPERIENCE
        )
        project = sum(
            package.line_cost for package in packages if package.kind == EntityKind.PROJECT
        )
        return (
    ...
    def _marginal_utility(
        ...
        selected_signals = (
            set().union(*(item.signal_ids for item in selected)) if selected else set()
        )
        added_signal_utility = sum(
            signal.weight * 10
            for signal in role.signals
            if signal.id in package.signal_ids - selected_signals
        )
        repeated_signal_penalty = sum(
            signal.weight * 5
            for signal in role.signals
            if signal.id in package.signal_ids.intersection(selected_signals)
        )
        return (
            added_signal_utility
            + package.impact
            - repeated_signal_penalty
            - (package.line_cost * 1.5)
        )

    def _plan_utility(self, packages: list[EntryPackage], role: RoleClassification) -> float:
        signal_ids = (
            set().union(*(package.signal_ids for package in packages)) if packages else set()
        )
        signal_utility = sum(
            signal.weight * 10 for signal in role.signals if signal.id in signal_ids
        )
        return (
            signal_utility
            + sum(package.impact for package in packages)
            - (sum(package.line_cost for package in packages) * 1.5)
        )
    ...
    def _select_skills(
        ...
        declared = [
            skill for skill in profile.declared_skills if self._term_matches_role(skill, role)
        ]
    ...
    def _strategy(self, role: RoleClassification, fit: ProfileFitAssessment) -> ResumeStrategy:
        ...
        return ResumeStrategy(
            ...
            secondary_focuses=[
                family.value.replace("_", " ") for family in role.secondary_role_families
            ],
            de_emphasized_themes=[
                "evidence that does not add role-signal coverage after entry cost"
            ],
            rationale=(
                f"Prioritize verified evidence for {primary_signal.label}; "
                f"profile fit is {fit.status.value}."
            ),
        )
    ...
    def _build_decisions(
        ...
        for package in selected:
            decisions.append(
                Decision(
                    ...
                    evidence_ids=[
                        evidence_id
                        for record in package.candidates
                        for evidence_id in record.candidate.evidence_ids
                    ],
                    reason=(
                        f"Included {len(package.candidates)} bullet candidate(s) "
                        "because their role coverage "
        ...
        for entity_id in entities:
            if entity_id in selected_ids:
                continue
            reason = (
                "Removed because its relevant evidence did not justify opening a "
                "new entry after page cost."
        ...
        decisions.append(
            Decision(
                ...
                entity_id="document",
                reason=(
                    "Entry costs include headings, metadata, spacing, and bullet "
                    "capacity before content is selected."
                ),
    ...
    def _matching_declared_skill_signal_ids(
        self, skills: list[str], role: RoleClassification
    ) -> set[str]:
        return {
            signal.id
            for skill in skills
    ...
    def write(
        ...
        if plan.strategy is None:
            raise UnsupportedOpportunityError(
                "Cannot generate a resume without sufficient profile fit."
            )
        ...
        for candidate in plan.claim_candidates:
            if candidate.support == ClaimSupport.UNSUPPORTED:
                continue
            if (
                candidate.support == ClaimSupport.STRONG_INFERENCE_PENDING_REVIEW
                and candidate.id not in approved_claim_ids
            ):
            ...
            target = (
                experience_bullets
                if entity_kinds[candidate.entity_id] == EntityKind.EXPERIENCE
        ...
        for skill in plan.demonstrated_skills:
            if skill.category_id not in selected_category_ids:
                continue
            if (
                skill.support == ClaimSupport.STRONG_INFERENCE_PENDING_REVIEW
                and skill.id not in approved_claim_ids
            ):
                review_required.append(skill.id)
                review_pending_skills.append(skill)
                continue
            category = next(
                category for category in technical_skills if category.id == skill.category_id
            )
        ...
        return StructuredResume(
            ...
            if plan.selected_skill_categories
            ...
            hybrid_diagnostic=plan.hybrid_diagnostic,
        )
